...
use Canabelle\CMSTranslationBundle\Entity\LanguageCatalogue;
use Canabelle\CMSTranslationBundle\Entity\LanguageToken;
use Canabelle\CMSTranslationBundle\Entity\LanguageTranslation;
...
class UpdateSystemTranslationsCommand extends ContainerAwareCommand
{
    ...
    {
        $this
            ->setName('canabelle:cms:translation:update-system-translations')
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        foreach ($data[0] as $catalogue) {
            $cat = $em->getRepository('CanabelleCMSTranslationBundle:LanguageCatalogue')->findOneBy(['name' => $catalogue['name']]);
            if (!$cat) {
        ...
        foreach ($catalogues as $cat) {
            foreach ($cat['tokens'] as $record) {
                $token = $em->getRepository('CanabelleCMSTranslationBundle:LanguageToken')->findOneBy(['catalogue' => $cat['cat'], 'token' => $record['token']['token']]);
                if (!$token) {
                ...
                foreach ($record['trans'] as $tr) {
                    $trans = $em->getRepository('CanabelleCMSTranslationBundle:LanguageTranslation')->findOneBy(['languageToken' => $token, 'language' => $tr['language']]);
                    if (!$trans) {
