...
class ImportTokensCommand extends ContainerAwareCommand
{
    ...
    {
        $this
            ->setName('canabelle:cms:translation:import-tokens')
        ...
        foreach ($srcFinder as $file) {
            ...
            foreach ($sites as $site_key => $site) {
                ...
                if ($add && !in_array($tokenName, $services) && !in_array($tokenName, $tokens)) {
                    ...
                    if (strpos($tokenName, '%') === false) {
                        $catalogue = $em->getRepository("CanabelleCMSTranslationBundle:LanguageCatalogue")->findOneByName($catalogueName);
                        if (!$catalogue) {
                        ...
                        if (!$em->getRepository('CanabelleCMSTranslationBundle:LanguageToken')->findOneBy(['token' => $tokenName])) {
                            $tokens[] = $tokenName;
                            $token = new \Canabelle\CMSTranslationBundle\Entity\LanguageToken();
            ...
            if (preg_match('/\'orangegate\.(.*)\'/', $fileContent, $matches) || preg_match('/\"orangegate\.(.*)\"/', $fileContent, $matches)) {
                ...
                if (!in_array($tokenName, $services) && !in_array($tokenName, $tokens)) {
                    ...
                    if (strpos($tokenName, '%') === false) {
                        ...
                        if (!$em->getRepository('CanabelleCMSTranslationBundle:LanguageToken')->findOneBy(['catalogue' => $catalogue, 'token' => $tokenName])) {
                            $tokens[] = $tokenName;
                            $token = new \Canabelle\CMSTranslationBundle\Entity\LanguageToken();
                            $token->setToken($tokenName);
                            $token->setCatalogue($catalogue);
